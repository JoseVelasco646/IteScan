...
async def scan_ports_segment(hosts: list[str], ports: str = "1-1024", emit_progress: bool = True, scan_id: str = None, host_timeout: int = 45):
    results = []
    total = len(hosts)
...
async def scan_services(host: str, ports: str = "1-1024", timeout: int = 120):
    async with GLOBAL_NMAP_SEMAPHORE:
        try:
            ...
            print(f"Timeout scanning services on {host}")
            return []

async def scan_services_segment(hosts: list[str], ports: str = "1-1024", emit_progress: bool = True, scan_id: str = None, host_timeout: int = 120):
    results = []
...
async def detect_os(host: str):
    async with GLOBAL_NMAP_SEMAPHORE:
        try:
            ...
            print(f"Timeout detecting OS on {host}")
            return None

async def detect_os_segment(hosts: list[str], emit_progress: bool = True, scan_id: str = None, host_timeout: int = 60):
    total = len(hosts)
    ws_manager = get_ws_manager() if emit_progress else None
...
async def scan_mac(network: str, emit_progress: bool = True, scan_id: str = None, host_timeout: int = 8):
    ws_manager = get_ws_manager() if emit_progress else None
