...
const scanServicesSegment = async () => {
  ...
  startScan('service-scan', controller)

  try {
    const data = await scannerAPI.scanServicesSegment(hostsArray, ports.value, controller.signal, timeout.value, concurrency.value)
